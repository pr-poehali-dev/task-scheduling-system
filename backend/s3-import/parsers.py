...
def is_fio(s: str) -> bool:
    ...
    skip = ["фамили", "иниц", "итого", "всего", "наименование", "должност",
            ...
            "отпуск", "больничн", "прогул", "невыход", "праздник",
            "отгул", "командиров", "сокращ", "увольн", "дополнительн",
            "основной", "ежегодн", "декрет", "учеб", "неоплач"]
...
def parse_work_types(file_bytes: bytes) -> list[dict]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in all_rows:
            ...
            if not unit:
                for c in cells:
                    if any(uw in c.lower() for uw in ["м²","м2","м³","м3","шт","пог","га","тонн","кг","чел"]):
                        unit = c
                        break

            results.append({"code": code, "name": name, "group_name": group_name, "unit": unit, "norm_per_hour": norm})
...
def parse_timesheet(file_bytes: bytes) -> list[dict]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row_i, row in enumerate(all_rows):
            cells = [safe_str(c) for c in row]
            day_nums = [(ci, safe_int(c)) for ci, c in enumerate(cells) if safe_int(c) is not None and 1 <= (safe_int(c) or 0) <= 31]
            if len(day_nums) >= 20:
...
def parse_orders(file_bytes: bytes) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    wb = load_workbook(BytesIO(file_bytes), read_only=True, data_only=True)
    all_orders, all_brigades, all_lines = [], [], []
    ...
    for sheet_idx, sheet_name in enumerate(wb.sheetnames):
        ...
        for row in all_rows[:40]:
            ...
            text = " ".join(cells)
            if not order_date:
                dm = re.search(r'(\d{1,2})\s+(январ|феврал|март|апрел|мая?|июн|июл|август|сентябр|октябр|ноябр|декабр)\S*\s+(\d{4})', text, re.IGNORECASE)
                if dm:
                    try:
                        mon_word = dm.group(2).lower()
                        mon_n = next((v for k,v in MONTH_MAP.items() if mon_word.startswith(k[:4])), 1)
                        order_date = dt.date(int(dm.group(3)), mon_n, int(dm.group(1)))
                    except Exception:
                        pass
        ...
        all_brigades.append({"order_number": order_number, "brigade_number": 1, "brigade_id_local": brigade_local_id})
        ...
        for row in all_rows:
            ...
            if bm:
                ...
                if brigade_local_id not in brigade_seen:
                    ...
                    all_brigades.append({"order_number": order_number, "brigade_number": current_brigade_num, "brigade_id_local": brigade_local_id})
                continue
            ...
            for ci in range(max_time_col+1, len(cells)):
                n = safe_int(cells[ci])
                if n and 1 <= n <= 24:
                    if plan_h is None: plan_h = float(n)
                    elif fact_h is None: fact_h = float(n); break
